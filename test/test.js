...
tape( 'the function has an arity of 2', function test( t ) {
	t.strictEqual( mskmin.length, 2, 'returns expected value' );
	t.end();
});
...
tape( 'the function calculates the minimum value of an array (accessors)', function test( t ) {
	...
	x = [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ];
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = mskmin( toAccessorArray( x ), toAccessorArray( mask ) );
	t.strictEqual( v, -4.0, 'returns expected value' );
...
tape( 'if provided an empty array, the function returns `NaN` (accessors)', function test( t ) {
	var v = mskmin( toAccessorArray( [] ), toAccessorArray( [] ) );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an array containing a single element, the function returns the first element (accessors)', function test( t ) {
	var v = mskmin( toAccessorArray( [ 1.0 ] ), toAccessorArray( [ 0 ] ) );
	t.strictEqual( v, 1.0, 'returns expected value' );
	t.end();
});
